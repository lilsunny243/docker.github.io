...
func (s *AwsCloudfrontUpdateCmd) Run() error {
	...
	if cfrtDistribConfig.DistributionConfig.DefaultCacheBehavior == nil {
		log.Printf("INFO: cloudfront distribution default cache behavior not found. skipping...")
		return nil
	}

	for i, funcAssoc := range cfrtDistribConfig.DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations.Items {
		...
		cfrtDistribConfig.DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations.Items[i].LambdaFunctionARN = publishConfig.FunctionArn
	}
	...
	_, err = cfrt.UpdateDistribution(&cloudfront.UpdateDistributionInput{
		...
		DistributionConfig: cfrtDistribConfig.DistributionConfig,
	})
	if err != nil {
		return err
	}
